...
try {
    ...
    const Usert = await User.findOne({where:{
        email:email
    }})
    const userType = Usert.user_type
     ...
     let files = 0;
        ...
        res.status(200).json({ workspaceCount: workspaceCount ,TeamSpace:Teamspace,folders:folders,files:files});
      } catch (error) {
        console.error(error);
    ...
    //   const user_id = 31
            ...
            const startingDate = Math.floor((currentDate.getTime() - 6 * 24 * 60 * 60 * 1000) / 1000); // Last 6 days in epoch seconds
            const endingDate = Math.floor((currentDate.getTime() + 1 * 24 * 60 * 60 * 1000) / 1000); // Tomorrow in epoch seconds
            ...
            if (userType === "Admin") {
                const latestFiless = await Fileupload.findAll({
                  ...
                  // Add email to each file in the latestFiles response
                   latestFiles = latestFiless.map(file => ({
                    ...
                    email: userMap[file.user_id] // Add the email property
                  }));
                ...
                latestFolders = latestFolderss.map(folder => ({
                    ...
                    email: userMapForFolders[folder.user_id] // Add the email property
                  }));
